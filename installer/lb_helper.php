...
class LicenseBoxAPI{
	...
	public function __construct(){ 
		$this->product_id = 'FA011ACB';
		$this->api_url = 'https://licencefly.com/';
		$this->api_key = 'EB632EDFB1F294-eventright';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["eadcd6d3626d0bd"])){
				$_SESSION["eadcd6d3626d0bd"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["eadcd6d3626d0bd"])){
				$get_data = $this->call_api(
					'POST',
